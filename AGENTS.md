...
基于 FastAPI + Codex Agent SDK 的 AI Agent 服务。支持 Skill 扩展、插件化 Channel 集成、多租户、动态模型供应商切换（Codex / Codex-router / glm / litellm）。
...
- `AGENT_CWD`（默认 `agent_cwd/`）— Agent 工作目录，Skills 和知识库在此目录下
  - `.Codex/skills/` — Skill 定义（`SKILL.md`）
  - `data/kb/` — 知识库文件
  - `data/tenants/` — 租户数据
...
请求入口 `POST /api/query` → `api/routers/agent.py` → `AgentService` → Codex SDK → `StreamProcessor` 输出 SSE。
...
- Codex Router 指 [Codex-router](https://github.com/musistudio/Codex-router)（ccr），使用前需 `eval "$(ccr activate)"`
