...
- 大需求（Deep Lane 新增业务能力）：①（父 Issue，约 9 分钟）→ 拆子 Issue → 每个子 Issue 独立跑 ③④⑤（约 8-10 分钟/个）
- 子 Issue PRD 生成：Step 5 分段生成（part1→part2→Python 合并），无 504，PRD 自动回填到子 Issue 描述
- 小需求 PRD 回填：PRD 内容自动追加到原 Issue 描述末尾，用 `---PRD文档---` 包裹，已有则替换
- 阶段 A/B（Issue 骨架创建 + PRD 回填到子 Issue）：已验证，PRD 回填到子 Issue 描述正常

**Step 5 分段生成机制（workflow_orchestrator.py `_run_step5`）：**
- PRD-A（有映射报告）：拆成 2 次 Agent 调用 + 1 次 Python 合并
  - 第一段：生成第一至三章，写 `{特性ID}_prd_part1.md`，写完立即结束
  - 第二段：生成第四至六章，写 `{特性ID}_prd_part2.md`，写完立即结束
  - Python 合并：直接读两个临时文件拼接，写 `{特性ID}_产品规格说明书_v1.0.md`，删临时文件
- PRD-B（无映射报告）：内容少，直接一次生成
- 合并用 Python 而非 Agent，避免合并阶段 504

**PRD 回填 bug 修复（handler.py `_trigger_phase2`）：**
- 子 Issue 目录含父 Issue 复制来的特性清单，旧逻辑 `not feature_list_files` 导致回填被跳过 → 已修复为直接扫根目录 PRD 文件
- 新文件名 `*_产品规格说明书_v*.md` 需同时加入 glob 模式（旧模式只有 `*_用户故事设计规格说明书_v*.md`）
...
- `_invoke_skill` 通过 `metadata["skill_file_hint"]` 注入 skill 文件完整路径，避免 Agent 因目录名有数字前缀（如 `02-ontology-context`）而 Glob 失败触发全局扫描（每步节省 ~15s）
- skill 查找机制：SDK 传 `skills=["ontology-context"]` → CLI 参数 `--allowedTools Skill(ontology-context)`；Agent 调用 Skill 工具时 CLI 按 SKILL.md 的 `name:` 字段匹配，不是目录名
- `_SKILL_PATH_MAP` 维护 skill 名称 → 实际文件路径的映射，新增 skill 时需同步更新
...
- Step 5 分段生成 prompt 中禁止使用中文引号（`""`），否则 f-string 语法错误导致 linear 插件加载失败
- 子 Issue PRD 回填被跳过：子 Issue 目录含父 Issue 复制来的特性清单，旧逻辑 `not feature_list_files` 判断失效 → 改为直接扫根目录 PRD 文件
- PRD 文件名从 `*_用户故事设计规格说明书_v*.md` 改为 `*_产品规格说明书_v1.0.md`，所有 glob 模式需同时兼容两种命名
