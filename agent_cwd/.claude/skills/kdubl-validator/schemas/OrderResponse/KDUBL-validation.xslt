...
<xsl:transform xmlns:cac="urn:oasis:names:specification:ubl:schema:xsd:CommonAggregateComponents-2"
	...
	<!-- Main template -->
	<xsl:template match="/">
		<svrl:schematron-output title="KDUBL 2.1 OrderResponse Validation Rules" schemaVersion="iso">
			...
			<svrl:active-pattern id="KDUBL-order-response-patterns" name="KDUBL-order-response-patterns"/>
			<xsl:apply-templates select="/" mode="M1"/>
		</svrl:schematron-output>
	</xsl:template>

	<!-- Mode M1: Document level validation -->
	<xsl:template match="ubl:OrderResponse" priority="1000" mode="M1">
		...
		<!-- OrderResponse-specific validation rules can be added here in the future -->
	</xsl:template>
