...
<xsl:transform xmlns:cac="urn:oasis:names:specification:ubl:schema:xsd:CommonAggregateComponents-2"
	...
	<!-- Main template -->
	<xsl:template match="/">
		<svrl:schematron-output title="KDUBL 2.1 OrderChange Validation Rules" schemaVersion="iso">
			...
			<svrl:active-pattern id="KDUBL-order-change-patterns" name="KDUBL-order-change-patterns"/>
			<xsl:apply-templates select="/" mode="M1"/>
		</svrl:schematron-output>
	</xsl:template>

	<!-- Mode M1: Document level validation -->
	<xsl:template match="ubl:OrderChange" priority="1000" mode="M1">
		...
		<!-- OrderChange-specific validation rules can be added here in the future -->
	</xsl:template>
