...
<xsl:transform xmlns:cac="urn:oasis:names:specification:ubl:schema:xsd:CommonAggregateComponents-2"
	...
	<!-- Main template -->
	<xsl:template match="/">
		<svrl:schematron-output title="KDUBL 2.1 OrderCancellation Validation Rules" schemaVersion="iso">
			<svrl:ns-prefix-in-attribute-values uri="urn:oasis:names:specification:ubl:schema:xsd:CommonBasicComponents-2" prefix="cbc"/>
			<svrl:ns-prefix-in-attribute-values uri="urn:oasis:names:specification:ubl:schema:xsd:CommonAggregateComponents-2" prefix="cac"/>
			<svrl:ns-prefix-in-attribute-values uri="urn:oasis:names:specification:ubl:schema:xsd:OrderCancellation-2" prefix="ubl"/>

			<svrl:active-pattern id="KDUBL-order-cancellation-patterns" name="KDUBL-order-cancellation-patterns"/>
			<xsl:apply-templates select="/" mode="M1"/>
		</svrl:schematron-output>
	</xsl:template>

	<!-- Mode M1: Document level validation -->
	<xsl:template match="ubl:OrderCancellation" priority="1000" mode="M1">
		...
		<!-- OrderCancellation-specific validation rules can be added here in the future -->
	</xsl:template>
