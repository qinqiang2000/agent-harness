...
<xsl:transform xmlns:cac="urn:oasis:names:specification:ubl:schema:xsd:CommonAggregateComponents-2"
	...
	<!-- Main template -->
	<xsl:template match="/">
		<svrl:schematron-output title="KDUBL 2.1 Order Validation Rules" schemaVersion="iso">
			...
			<svrl:active-pattern id="KDUBL-order-patterns" name="KDUBL-order-patterns"/>
	...
	<xsl:template match="ubl:Order" priority="1000" mode="M1">
		...
		<!-- Order-specific validation rules can be added here in the future -->
	</xsl:template>
