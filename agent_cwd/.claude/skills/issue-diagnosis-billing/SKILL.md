...
- **「本地不存在」是结论，不是待验证的假设**：当检索已表明报错文案/功能逻辑不在本地代码库中（全库 grep 关键文案零匹配，或已确认某模块无相关实现），即可直接判定为外部系统问题并进入 Step 5 输出结论。**禁止**换关键词、换服务、翻接口文档反复自证「我们这边确实没有」——每多查一轮都不会改变结论，只消耗 turns。
  - 适用于主 agent 与子 agent。
  - 正确做法：在【证据】中写明「已在 X 范围内全量检索关键词 Y，零匹配」，结论定为 `EXTERNAL_ISSUE`（或问题归属外部时的 `BUSINESS_FAQ`），并在【解决建议】中指出应由哪个外部团队排查、需要对方提供什么信息。
  - 反例（真实案例，均超 30 turns / 5 分钟）：CNPRD-1534 全库检索「一票多用」零匹配后仍逐服务扩大搜索；CNPRD-1578 已确认发票云无 SRM 相关逻辑后仍反复查同一模块、接口文档与「是否有接口会拆行」。
...
> ⚠️ **多需求点时必须一次性并行搜索**：工单包含多个子需求点（N≥2）时，**必须在同一轮 Grep 调用中并行搜索所有需求点的关键词**，不得分批、分轮执行。每个需求点对应一个独立的 Grep 调用，所有调用同时发出，拿到结果后再统一 Read 源码文件。分批搜索会无谓增加 turns 消耗，影响响应速度。
