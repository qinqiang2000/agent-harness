...
description: >-
  开票/收票/影像业务问题诊断与根因定位。当用户提供 traceId、报错信息、异常堆栈，或描述开票/收票/影像服务异常时，
  通过「日志分析 + 源码联合定位」给出根因结论；当用户只描述业务疑问、没有任何错误线索时，从知识库检索答案。
  即使用户只是粘贴了一段错误日志、一个 traceId、或描述了某个接口异常，也应触发此 Skill。
  适用场景：开票失败、红冲失败、收票异常、查验不通过、影像归档失败、扫描异常、OFD/PDF解析失败、
  鉴权失败、token失效、参数校验失败、接口超时、服务超时、连接失败、发票状态异常等标准版产品相关问题。
  当用户描述标准版产品相关问题时，必须使用此 Skill，即使问题描述比较模糊或未明确指出是开票/收票/影像。
---
...
**路径 B — 无可查线索**（无 traceId，也无任何报错/关键字，仅描述业务疑问）：
...
输出结论后，**立即判断**根因是否为代码问题。满足以下任意条件即视为代码问题：
- 【根因分析】中出现"代码"、"逻辑"、"字段赋值"、"空指针"、"NPE"、"枚举"、"Feign"、"未传"、"注入"、"参数缺失"、"异常类"、"堆栈"等字样
- 【解决建议】中包含"修改代码"、"检查代码"、"修复逻辑"、"排查代码"等字样
- 【证据】中包含 `源码: {ClassName}.java:{行号}` 格式的定位信息

**判断为代码问题时**：必须紧接着调用 `Skill("code-fix", "{服务名}:{根因一句话摘要}")` 自动触发修复，不等用户指令，直接执行。

**判断为非代码问题时**（纯配置、纯数据、纯外部系统问题）：不触发 code-fix。
