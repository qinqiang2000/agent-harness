...
**优先使用子 Agent 做源码分析**，避免大量源码内容占用主 agent context 导致超限。

#### 何时使用子 Agent

满足以下任意一条时，必须通过 `Agent` tool 派发子 agent 执行源码分析：
- 需要跨 **2 个以上**文件做关联分析
- 预计需要读取的源码文件超过 **3 个**
- 调用链涉及多个服务（RPC 跨服务追踪）

仅单文件单方法的简单定位，可直接 Grep + Read，无需子 agent。

#### 子 Agent 调用规范

使用 `Agent` tool，**禁止指定 `subagent_type` 参数**（服务端不支持自定义类型，指定后子 agent 会立即退出不执行任何工具）。只传 `prompt`，让 SDK 使用默认类型。

**⚠️ 工具调用预算（防止子 agent 盲搜拖垮整个诊断，必须写入 prompt）：**
- 单个子 agent 的工具调用（Grep/Glob/Read/Bash）**累计不得超过 25 次**。达到上限仍未定位到根因，必须立即停止并返回「已达检索上限，当前最可能的方向是 XXX，建议补充 YYY 信息」，**禁止继续无边界试探**。
- 检索策略必须「先窄后宽」：先用报错类名/接口路径/方法名精确定位（Grep -l 找文件 → Read 目标方法），**只有精确检索连续 3 次落空**才允许扩大到 Glob 目录级扫描。
- **禁止在子 agent 内再派发 `local_bash` 后台任务查类定义**，类定义用 Grep/Read 直接读。
- 主 agent 侧：一次源码分析**只派发 1 个子 agent**，不要在子 agent 运行期间自己并行 grep/find 同一批文件（会与子 agent 重复且加剧超时）。

prompt 严格按以下格式（根据问题类型选对应模板）：

**模板 A — 有明确报错（日志有 ERROR/异常类/方法名）：**

```
你是源码分析专家，只做源码定位和分析，不做诊断结论。
工具调用累计上限 25 次，超限立即返回当前最可能方向，禁止无边界试探。先精确检索（类名/方法名），连续 3 次落空才允许 Glob 目录扫描。禁止派发后台任务。

代码根路径：{实际 CODE_BASE 路径}
分析目标服务：{repo-name}，代码目录：{实际代码目录路径}

问题背景（来自日志）：
- 报错信息：{从日志提取的关键报错}
- 关键字段值：{从日志提取的关键参数，如 clientId、invoiceNo 等}
- 疑似报错类：{ClassName}，方法：{methodName}

分析任务：
1. 定位 {ClassName}.{methodName} 的具体实现（文件路径:行号）
2. 分析该方法的调用链（上游调用者是谁，传了什么参数）
3. 结合日志中的实际数据，判断哪个输入值触发了异常
4. 判断根因：调用方传了非预期数据 / 被调用方返回非预期格式 / 代码本身缺少防御

返回格式（严格遵守，不要返回源码原文）：
- 关键类定位：{ClassName}.java:{行号} — {该行逻辑一句话描述}
- 调用链摘要：{上游} → {中间层} → {报错点}（每跳一行，不超过 6 跳）
- 触发条件：{具体是哪个字段/值/条件导致走到报错}
- 根因判断：{一句话结论}
```

**模板 B — 现象型问题（无报错，如"页面无数据/勾选后没了/查不到"）：**
此类问题没有异常栈可锚定，最易触发盲搜超时。**必须先让主 agent 从工单/前端接口提取 1 个明确的后端接口路径或 Controller 名**再派发，prompt 里给定入口，不要让子 agent 自己找入口。

```
你是源码分析专家，只做源码定位和分析，不做诊断结论。
工具调用累计上限 25 次，超限立即返回当前最可能方向，禁止无边界试探。禁止派发后台任务。

代码根路径：{实际 CODE_BASE 路径}
分析目标服务：{repo-name}，代码目录：{实际代码目录路径}

明确入口（主 agent 已确定，直接从此处开始，不要自己找入口）：
- 接口路径：{如 /m4/fpzs/expense/invoice/query}
- 入口类：{ControllerName}.java

问题现象：{一句话描述，如"企业票夹勾选后列表查询无数据"}

分析任务（严格按顺序，找到即返回，不要发散）：
1. 从入口类的该接口方法开始，只沿这一条查询链向下读（Controller → Service → DAO/SQL）
2. 定位最终数据查询处（SQL 或 QueryFilter 构造），列出全部过滤条件字段
3. 判断哪个过滤条件最可能把目标数据排除（如 status/fdelete/source/orgId/时间范围）

返回格式（严格遵守，不要返回源码原文）：
- 查询链：{Controller.method} → {Service.method} → {DAO/SQL 位置}（每跳一行，不超过 6 跳）
- 全部过滤条件：{字段名 = 值 的清单}
- 疑似根因：{哪个过滤条件导致数据被排除，一句话}
```

子 agent 返回摘要后，主 agent 基于摘要完成 4.3 联合分析，**不再重复读取源码**。
...
找到源码后（无论来自子 agent 摘要还是直接读取），必须回答以下四个问题，才算完成根因定位：
