...
- **不满足以上例外条件时，默认进入 Step 4 源码定位**，包括但不限于：
  ...
  - **日志含无法理解的枚举值或状态码**：出现形如 `xxxType=N`、`xxxStatus=N`、`errorCode=N`、`xxxSource=N` 的数字型字段，先查 [references/field-glossary.md](references/field-glossary.md)，命中则直接理解继续分析；**未命中且该字段含义影响根因判断** → 进入 Step 4，禁止自行猜测
