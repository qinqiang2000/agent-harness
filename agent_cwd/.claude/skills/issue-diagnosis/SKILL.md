...
**触发学习的信号**（满足任意一条即触发）：
- **显式否定**（置信度 0.7）：用户明确否定，如"不对"、"不是这个原因"、"你说错了"、"建议有问题"、"是不是不对"、"这个不准确"、"你漏了XXX"、"应该是XXX"
- **隐式纠正**（置信度 0.4）：用户补充了与上一轮结论矛盾的新信息，如"但是OCR其实成功了"、"实际上是XXX导致的"

**触发后执行**：
1. 按用户补充的信息重新诊断，得出正确结论
...
**严格禁止**：不得修改 FAQ 文件（`kb/` 目录）和 `SKILL.md`，只写 `data/issue-diagnosis/instincts/cases.md`。
