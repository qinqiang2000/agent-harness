...
输出结论后，**必须立即判断**根因是否为代码问题。满足以下任意条件即视为代码问题：
...
**判断为代码问题时**：输出结论后，**必须**紧接着调用 `Skill("code-fix", "{服务名}:{根因一句话摘要}")` 自动触发修复。这一步是强制的，不等用户指令，不询问用户是否需要修复，直接执行。

**判断为非代码问题时**（纯配置、纯数据、纯外部系统问题）：不触发 code-fix，只输出确认请求。
