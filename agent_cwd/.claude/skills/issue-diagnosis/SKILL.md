...
- **重试上限强制执行**：无论换关键词还是换环境，累计查询次数超过初次 + 2 次重试后，必须立即停止并反问用户，不得以任何理由继续查询
...
| 收票 | 收票、查验、OFD、PDF、邮箱取票、邮箱绑定、合规校验、发票助手、台账、抵扣计算、重构版、导入发票、重复报销、已被报销、发票已关联、个人发票、个人抬头、个人票、抬头不一致、税号不一致、查验不通过、红冲、作废、超期、跨年、未上传源文件、黑名单、敏感词、连号 | [kb/收票-faq.md](kb/收票-faq.md) |
...
查询返回后，严格按 [references/log-analysis.md](references/log-analysis.md) 中的「查询后处理」和「日志读取规则」执行。
...
**⚠️ SQL 安全约束**：详见 [references/db-queries.md](references/db-queries.md) 顶部约束说明，此处不重复。
...
3. 将模板中的占位符替换为实际参数值后执行：
   ...
   python3 .claude/skills/issue-diagnosis/scripts/cosmic_query.py \
     ...
     --sql "SELECT ... WHERE field = ?" \
     --params '["<值>"]' \
     --evidence "<FAQ文件名，如收票-faq.md>" \
     --source-type faq
   ```
   **SQL 中条件值必须用 `?` 占位，通过 `--params` 传入 JSON 数组，服务端用 PreparedStatement 绑定防止 SQL 注入。表名无需手动加库名前缀，服务端自动注入。服务端会执行 EXPLAIN 预检，发现全表扫描或慢查询特征时拒绝执行。**
4. 将查询结果纳入最终诊断报告
...
结论输出完毕后，判断是否满足以下全部条件：
- 本次诊断走过 Step 4 源码定位
- 根因结论有源码级证据（具体类名/方法名/行号）
- 根因是代码逻辑缺陷，而非纯配置/数据/外部系统问题

**满足**：在同一轮输出末尾追加：
> 以上分析是否帮助您解决了问题？请回复 1 解决  2 未解决。
>
> 另外，本次根因定位到代码逻辑层面，是否需要启动自动修复流水线？同意后将创建一张 Linear bug 单，您在 Linear 审核确认后系统会自动进行 TDD 修复并提交 MR（合并到主干仍需人工）。请回复 **是** 或 **否**。

收到用户回复后：
- 回复「是」→ 读取 [references/create-repair-issue.md](references/create-repair-issue.md)，完整按其步骤执行。写 payload 时，从本次诊断过程中所有日志的 `project` 字段和源码定位结果收集受影响服务列表，填入 `affected_services`，至少包含根因所在服务。完成后进入 Step 7。
- 回复「否」或仅回复「1/2」→ 跳过提单，进入 Step 7。
