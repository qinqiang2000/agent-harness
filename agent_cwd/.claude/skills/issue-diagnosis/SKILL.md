...
# 通用问题诊断
**严格按照步骤要求执行**
**⚠️ 全程禁止在任何输出中暴露执行步骤**：不得在任何时候（包括中间过程的文字输出）出现"Step 1"、"Step 2"等步骤标题。所有步骤静默执行，只在 Step 6 输出最终结论。
...
检查 `data/issue-diagnosis/instincts/cases.md` 是否存在。若存在，读取文件，用用户描述的关键词（报错类型、服务名、关键字段、错误码）与每条 case 的「适用条件」做语义匹配。

- **命中置信度 >= 0.6 的 case**：在后续诊断中优先按该 case 的「正确路径」执行，并在 Step 6 输出结论时注明"参考历史经验 Case #XXX"
- **未命中或文件不存在**：继续 Step 2，按标准流程执行
...
**直接调用 `mcp__elastic__searchTraceOrKeyWordsLog`**（禁止用任何工具搜索该工具名，直接使用），按全局查询规范构建参数，从返回日志的 `fields.project` 字段获取服务名（供源码定位使用）。
...
【证据】
- 日志：TraceId {traceId}，时间 {timestamp}，服务 {fields.project}
  关键日志行：{直接支撑根因的日志内容}
...
**触发学习的信号**：
- **显式反馈**（置信度 0.7）：用户明确否定结论，如"不对"、"不是这个原因"、"应该是XXX"、"你漏了XXX"
- **隐式反馈**（置信度 0.4）：用户在同一会话内追问同一问题，如"再查一下"、"还有别的原因吗"、补充了新的错误信息

**触发后执行**：
1. 按用户补充的信息重新诊断，得出正确路径
2. 将完整案例追加写入 `data/issue-diagnosis/instincts/cases.md`（文件不存在则创建）：

```markdown
## Case #XXX
- 触发场景: {用户描述的问题，含关键词/报错/服务名}
- 初次诊断: {第一次给出的根因结论}
- 用户反馈: [{显式|隐式}] {用户的原话或补充信息}
- 正确路径: {重新诊断后的正确根因和定位步骤}
...
3. 告知用户："已记录本次经验，下次遇到类似问题会优先参考。"

**严格禁止**：不得修改 FAQ 文件（`kb/` 目录）和 `SKILL.md`，只写 `instincts/cases.md`。
