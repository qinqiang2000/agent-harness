...
- 本次诊断走过 Step 4 源码定位，且
- 根因结论有源码级证据（具体类名/方法名/行号），且
- 根因是代码逻辑缺陷（如字段解析错误、条件判断错误、枚举映射错误等），而非纯配置/数据/外部系统问题
...
```json
{
  "team_key": "<团队标识符，如 ARALGO、QUASAF，从仓库路径或服务所属团队判断>",
...
`team_key` 填团队标识符（即 Linear issue 编号前缀，如 ARALGO-34 对应 `ARALGO`），CLI 会自动解析成团队 ID。`repair_plan` 只写修复**方向**（哪个类/方法、加什么校验），不写完整代码。
