...
> ⚠️ **安全约束（严格执行，违反会导致生产数据库崩溃）**
> - **禁止自行构造 SQL**：只能使用本文档中的预定义模板，原样替换占位符后执行
> - **禁止 `LIKE '%xxx%'` 模糊匹配**：全表扫描，在大表上会直接拖垮生产库
> - **禁止查询无索引字段**：所有模板的 WHERE 条件均已使用索引字段，不得修改
> - **禁止无 WHERE 条件的全表查询**：每条查询必须有明确的过滤条件
> - **没有匹配模板时必须跳过**：不得尝试自行编写 SQL，直接进入 Step 6 输出结论
