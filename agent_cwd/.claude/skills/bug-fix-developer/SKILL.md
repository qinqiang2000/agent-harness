...
**最后必须输出【状态】**：只有当代码已改、已 commit 并成功 `git push`（建了 MR 或至少推上分支）时，才填「完成」；
任何中途卡住（如改不动、push 失败、缺 token、被权限拦、放弃）一律填「失败」，并在【说明】里讲清卡在哪。
coordinator 仅在【状态】完成且有【分支】时才触发构建；填「失败」则不构建、转人工。**不要为了凑格式谎报完成。**
...
【自审】
- [级别] 问题描述
【说明】<一句话总结这次修复做了什么；若【状态】失败则说明卡在哪>
```
