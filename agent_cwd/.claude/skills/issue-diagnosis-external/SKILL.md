...
- **参数构建、重试策略、时间范围**：严格按 [.claude/skills/issue-diagnosis/references/query-strategy.md](.claude/skills/issue-diagnosis/references/query-strategy.md) 执行
- **查询后处理、深度分析**：严格按 [.claude/skills/issue-diagnosis/references/log-analysis.md](.claude/skills/issue-diagnosis/references/log-analysis.md) 执行
...
| 场景 | 识别条件 | 跳转 |
|---|---|---|
| 进项发票采集任务 | 用户提到"进项发票采集任务"、"任务号"、"批次号"、"batchNo"、"任务状态"、"任务失败"、"任务处理中" | [.claude/skills/issue-diagnosis/references/invoice-task-diagnosis.md](.claude/skills/issue-diagnosis/references/invoice-task-diagnosis.md) |
| 接口代码查阅 | 用户询问某接口的处理流程、代码位置，或提到"接口"+"业务名称"（如"查验接口"、"开票接口"），且 `data/kb/接口文档/` 目录存在 | [.claude/skills/issue-diagnosis/references/api-lookup.md](.claude/skills/issue-diagnosis/references/api-lookup.md) |
...
| 领域 | 触发关键词 | 文件 |
|---|---|---|
| 开票 | 开票、开具、红冲、票种 | [.claude/skills/issue-diagnosis/kb/开票-faq.md](.claude/skills/issue-diagnosis/kb/开票-faq.md) |
| 收票 | 收票、查验、OFD、PDF、邮箱取票、邮箱绑定、合规校验、发票助手、台账、重构版、导入发票、重复报销、已被报销、发票已关联、个人发票、个人抬头、个人票、抬头不一致、税号不一致、查验不通过、红冲、作废、超期、跨年、未上传源文件、黑名单、敏感词、连号 | [.claude/skills/issue-diagnosis/kb/收票-faq.md](.claude/skills/issue-diagnosis/kb/收票-faq.md) |
| 鉴权登录 | 登录、鉴权、token、clientId、验证码、二维码 | [.claude/skills/issue-diagnosis/kb/鉴权登录-faq.md](.claude/skills/issue-diagnosis/kb/鉴权登录-faq.md) |
| 接口参数 | 参数、字段、格式、必填、校验、不合法 | [.claude/skills/issue-diagnosis/kb/接口参数-faq.md](.claude/skills/issue-diagnosis/kb/接口参数-faq.md) |
| 进项发票采集 | 进项、采集、下票、全量查询、表头、抵扣勾选、退税勾选、入账、海关缴款书、版式文件下载 | [.claude/skills/issue-diagnosis/kb/进项发票采集-faq.md](.claude/skills/issue-diagnosis/kb/进项发票采集-faq.md) |
| 性能超时 | 超时、timeout、慢、连接失败、积压 | [.claude/skills/issue-diagnosis/kb/性能超时-faq.md](.claude/skills/issue-diagnosis/kb/性能超时-faq.md) |
...
- **条目不含「诊断步骤」，且有 traceId 或足够关键词** → 记录 FAQ 答案中的**核心判断条件**（如"clientId 是否 TN_ 开头"、"字段是否为空"等），进入 Step 3 时**必须以这些条件为验证目标**，在日志中明确确认或否定每一个条件
...
查询返回后，严格按 [.claude/skills/issue-diagnosis/references/log-analysis.md](.claude/skills/issue-diagnosis/references/log-analysis.md) 中的「查询后处理」和「日志读取规则」执行。
...
必须严格按 [.claude/skills/issue-diagnosis/references/gitlab-lookup.md](.claude/skills/issue-diagnosis/references/gitlab-lookup.md) 的顺序执行：
1. 读 `.claude/skills/issue-diagnosis/references/service-repo-map.md`，用日志中的 `project` 精确匹配，获取 `project_id`
2. 映射表未命中时，跳过源码分析，直接凭日志分析给出结论，不要妄自猜测
3. 获得 `project_id` 后，**必须使用 `.claude/skills/issue-diagnosis/references/gitlab-lookup.md` 中的 clone 模板**，原样替换 `{repo-name}` 和 `{namespace/repo-name}`，**禁止自行简化命令或省略 `token:$GITLAB_TOKEN@`**
...
**⚠️ SQL 安全约束**：详见 [.claude/skills/issue-diagnosis/references/db-queries.md](.claude/skills/issue-diagnosis/references/db-queries.md) 顶部约束说明，此处不重复。

**其他严禁行为**：根据日志信号自行推断表名或字段；自行探索数据库表结构（SHOW TABLES、INFORMATION_SCHEMA 等）；日志查不到时用数据库查询替代日志分析；查询结果为空时自行扩展查询其他表、其他字段或其他数据源。**查完即止**。

**数据源选择规则**（通过 `--env` 和 `--db` 参数指定）：
- 用户指明"测试环境"/"sit 环境" → `--env sit`；用户指明"演示环境"/"demo 环境" → `--env demo`；未指明或指明"生产环境" → `--env prod`（默认）
- 开票/鉴权相关问题 → `--db cms`
- 运营/订单相关问题 → `--db eop`
- 收票合规校验相关问题 → `--db invoice`

**执行步骤**：
1. 读取 [.claude/skills/issue-diagnosis/references/db-queries.md](.claude/skills/issue-diagnosis/references/db-queries.md)，根据 FAQ 条目编号找到对应模板
2. 若无匹配模板 → 跳过，直接进入 Step 6
3. 将模板中的占位符替换为实际参数值后执行：
   ```bash
   python3 .claude/skills/issue-diagnosis/scripts/cosmic_query.py \
     --env <prod|sit|demo> \
     --db <数据库名，如 invoice、cms、eop> \
     --sql "SELECT ... WHERE field = ?" \
     --params '["<值>"]' \
     --evidence "<模板标题>" \
