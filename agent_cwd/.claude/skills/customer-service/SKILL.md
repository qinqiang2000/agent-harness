...
**MUST 在输出最终答案前验证产品线一致性：**

#### 校验流程

1. **记录用户产品**：用户明确的产品是 `{用户产品}`
2. **检查文档来源**：验证引用文档的路径或内容中的产品标识
3. **一致性判断**：若不一致 → **丢弃该文档，重新搜索正确产品的文档**
...
**通用文档**：不含任何产品标识的文档（如通用税务规则、乐企平台等）可用于所有产品。

#### 校验失败处理

若找到的文档与用户产品不匹配：
1. **丢弃该文档**
2. **重新搜索**：明确限定在用户产品的目录下搜索
3. **仍无结果**：返回标准话术"抱歉，在发票云知识库没找到本答案..."
