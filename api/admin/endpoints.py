...
from api.services.admin.sync_service import (
    sync_lock_manager,
    stream_basic_data_sync,
    stream_invoice_sync
)
from api.models.admin.sync_models import (
...
async def get_configs():
    ...
    configs = config_service.get_available_configs()
    current = config_service.get_current_config_name()
    env_snapshot = config_service.get_current_env_snapshot()
...
async def switch_config(request: SwitchConfigRequest) -> SwitchConfigResponse:
    ...
    config_service = get_config_service()
    success = config_service.switch_config(request.config_name)

    if success:
        return SwitchConfigResponse(
            success=True,
            message=f"Successfully switched to {request.config_name}",
            current_config=config_service.get_current_config_name(),
            env_snapshot=config_service.get_current_env_snapshot()
        )
    else:
        raise HTTPException(
