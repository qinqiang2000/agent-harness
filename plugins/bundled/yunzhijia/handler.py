...
class YunzhijiaHandler:
    ...
    # 云之家通知 URL 模板（从 YZJ_ALERT_WEBHOOK 提取 token 构建，无需单独配置）
    _alert_webhook = os.getenv("YZJ_ALERT_WEBHOOK", "")
    if "yzjtoken=" in _alert_webhook:
        # 从完整 URL 提取基础部分，替换 token 为 {} 占位
        import re as _re
        NOTIFY_URL_TEMPLATE = _re.sub(r'yzjtoken=[^&]+', 'yzjtoken={}', _alert_webhook)
    else:
        # fallback：用旧的独立配置或默认值
    ...
    FAQ_MAP = {
        "你好": "您好，我是运维机器人。您可以 @我 + 服务器IP 让我帮您诊断，如：诊断一下 172.31.16.40",
        "你能做什么": "我可以自动诊断服务器告警（CPU/内存/磁盘/IO），@我 + IP 即可开始",
        "能做什么": "我可以自动诊断服务器告警（CPU/内存/磁盘/IO），@我 + IP 即可开始",
    }
    ...
    async def _process_agent_stream(
        ...
        async for event in self.agent_service.process_query(request):
            ...
            if event_type == "session_created":
                ...
                logger.error(f"[YZJ] Agent error: {error_msg}")
                await self.message_sender.send_text(
